*** 
YAPILACAKLAR
* Exception' lar eklenecek

```sh
    Welcome! Game is starting...
    Enter your name: Zeynep
    The characters and their properties are shown below.
    
    Character	 	ID	Damage	Health	Money
    1-Samurai		 1		 5		21	   15
    2-Archer		 2		 7		18	   20
    3-Knight		 3		 8		24	    5
    
    Please choose your character: 3
    Character{id=3, damage=8, health=24, money=5, name='Knight'}
    
    
    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 1

	=====================================
  	Location	Obstacle	Booty
	1-Cave		Zombie		Food
	2-Forest	Vampire		Firewood
	3-River		Bear		Water
	4-Mine		Snake		Joker
	5-Back
	=====================================
    Please enter the name of location: 2
    There are 1 Vampire(s)
    The health of Zeynep --> 24
    The health of Vampire --> 14
    Do you want to fight? (yes / no): yes

	*** Vampire shoot Zeynep ***
	The health of Zeynep --> 20
	The health of Vampire --> 14

	*** Zeynep shoot Vampire ***
	The health of Zeynep --> 20
	The health of Vampire --> 6

	*** Vampire shoot Zeynep ***
	The health of Zeynep --> 16
	The health of Vampire --> 6

	*** Zeynep shoot Vampire ***
	The health of Zeynep --> 16
	The health of Vampire --> 0

	**************************
	Vampire(s) dead.
	Character{id=3, damage=8, health=16, money=12, name='Knight'}

    Please enter the name of location: 1
    There are 2 Zombie(s)
    The health of Zeynep --> 16
    The health of Zombie --> 20
    Do you want to fight? (yes / no): yes

	*** Zombie shoot Zeynep ***
	The health of Zeynep --> 13
	The health of Zombie --> 20

	*** Zeynep shoot Zombie ***
	The health of Zeynep --> 13
	The health of Zombie --> 12

	*** Zombie shoot Zeynep ***
	The health of Zeynep --> 10
	The health of Zombie --> 12

	*** Zeynep shoot Zombie ***
	The health of Zeynep --> 10
	The health of Zombie --> 4

	*** Zombie shoot Zeynep ***
	The health of Zeynep --> 7
	The health of Zombie --> 4

	*** Zeynep shoot Zombie ***
	The health of Zeynep --> 7
	The health of Zombie --> 0

	**************************
	Zombie(s) dead.
	Character{id=3, damage=8, health=7, money=20, name='Knight'}

    Please enter the name of location: 2
    You have Firewood, you can not visit Forest
    
    Please enter the name of location: 3
    There are 2 Bear(s)
    The health of Zeynep --> 7
    The health of Bear --> 40
    Do you want to fight? (yes / no): no
    
    Please enter the name of location: 5
    
    
    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 2

	=====================================
	Location	 	Feature
	1-Safe House	Restore
	2-Tool Store	Buy Weapon/Defence
	3-Back
	=====================================
    Please enter the name of location: 2

	****** Welcome to the Tool Store ******
		The items on the Tool Store: 
	1-Armors --> get defence
	2-Weapons --> get damage
	3-Exit
    Please choose a item: 1

	Armors	 	ID	Defence 	Money
	1-Light		 1		 1		15
	2-Medium	 2		 3		25
	3-Heavy		 3		 5		40
	4-Back
	Choose an item: 1

	Your money: 20
	Buy or Leave: leave


    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 2

	=====================================
	Location	 	Feature
	1-Safe House	Restore
	2-Tool Store	Buy Weapon/Defence
	3-Back
	=====================================
    Please enter the name of location: 2

	****** Welcome to the Tool Store ******
		The items on the Tool Store: 
	1-Armors --> get defence
	2-Weapons --> get damage
	3-Exit
    Please choose a item: 2

	Weapons	 	ID	Damage	Money
	1-Gun		 1		 2		25
	2-Sword		 2		 3		35
	3-Rifle		 3		 7		45
	4-Back
	Choose an item: 2

	Your money: 20
	Buy or Leave: buy
    You don't have enough money


    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 2

	=====================================
	Location	 	Feature
	1-Safe House	Restore
	2-Tool Store	Buy Weapon/Defence
	3-Back
	=====================================
    Please enter the name of location: 1

    Restoring...Character{id=3, damage=8, health=24, money=20, name='Knight'}


    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 1

	=====================================
  	Location	Obstacle	Booty
	1-Cave		Zombie		Food
	2-Forest	Vampire		Firewood
	3-River		Bear		Water
	4-Mine		Snake		Joker
	5-Back
	=====================================
    Please enter the name of location: 3
    There are 3 Bear(s)
    The health of Zeynep --> 24
    The health of Bear --> 60
    Do you want to fight? (yes / no): no
    
    Please enter the name of location: 4
    There are 2 Snake(s)
    The health of Zeynep --> 24
    The health of Snake --> 24
    Do you want to fight? (yes / no): yes

	*** Snake shoot Zeynep ***
	The health of Zeynep --> 19
	The health of Snake --> 24

	*** Zeynep shoot Snake ***
	The health of Zeynep --> 19
	The health of Snake --> 16

	*** Snake shoot Zeynep ***
	The health of Zeynep --> 14
	The health of Snake --> 16

	*** Zeynep shoot Snake ***
	The health of Zeynep --> 14
	The health of Snake --> 8

	*** Snake shoot Zeynep ***
	The health of Zeynep --> 9
	The health of Snake --> 8

	*** Zeynep shoot Snake ***
	The health of Zeynep --> 9
	The health of Snake --> 0

	**************************
	Snake(s) dead.
	Character{id=3, damage=8, health=9, money=20, name='Knight'}
	You won nothing

    Please enter the name of location: 3
    There are 1 Bear(s)
    The health of Zeynep --> 9
    The health of Bear --> 20
    Do you want to fight? (yes / no): no
    
    Please enter the name of location: 5
    
    
    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 2

	=====================================
	Location	 	Feature
	1-Safe House	Restore
	2-Tool Store	Buy Weapon/Defence
	3-Back
	=====================================
    Please enter the name of location: 1

    Restoring...Character{id=3, damage=8, health=24, money=20, name='Knight'}


    LOCATION
    1-Battle Location
    2-Normal Location
    3-Exit
    Please choose your location where you want to go: 1

	=====================================
  	Location	Obstacle	Booty
	1-Cave		Zombie		Food
	2-Forest	Vampire		Firewood
	3-River		Bear		Water
	4-Mine		Snake		Joker
	5-Back
	=====================================
    Please enter the name of location: 3
    There are 2 Bear(s)
    The health of Zeynep --> 24
    The health of Bear --> 40
    Do you want to fight? (yes / no): yes

	*** Zeynep shoot Bear ***
	The health of Zeynep --> 24
	The health of Bear --> 32

	*** Bear shoot Zeynep ***
	The health of Zeynep --> 17
	The health of Bear --> 32

	*** Zeynep shoot Bear ***
	The health of Zeynep --> 17
	The health of Bear --> 24

	*** Bear shoot Zeynep ***
	The health of Zeynep --> 10
	The health of Bear --> 24

	*** Zeynep shoot Bear ***
	The health of Zeynep --> 10
	The health of Bear --> 16

	*** Bear shoot Zeynep ***
	The health of Zeynep --> 3
	The health of Bear --> 16

	*** Zeynep shoot Bear ***
	The health of Zeynep --> 3
	The health of Bear --> 8

	*** Bear shoot Zeynep ***
	The health of Zeynep --> 0
	The health of Bear --> 8
	
	...GAME OVER!!!